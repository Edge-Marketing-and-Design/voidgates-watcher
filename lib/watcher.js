// TODO TEST CLEANUP, AND ZONE SPECIFIC BLOCKS, MAKE SURE SERVER IS GETTING REAL IP ADDRESSES AND NOT CLOUDFLARE PROXIES
...
const pathBlockDurationMs = Number(PATH_BLOCK_DURATION_SECONDS) * 1000
  ...
  while (hasMore) {
    try {
      ...
      for (const rule of entries) {
        ...
        if ((isPathBlock || is404Block) && rule.configuration?.target === 'ip') {
          console.log(rule)
          const timestamp = rule.notes.replace(`${isPathBlock ? PATH_BLOCK_NOTE_PREFIX : BLOCK_NOTE_PREFIX} `, '').trim()
          const expired = Date.now() - new Date(timestamp).getTime() > (isPathBlock ? pathBlockDurationMs : blockDurationMs)
